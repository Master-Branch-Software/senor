...
`eslint-plugin-jsx-a11y` catches the most common JSX accessibility mistakes (missing `alt`, non-interactive elements with click handlers, labels without `htmlFor`). It does not replace manual audits — see `accessibility.md` — but it prevents an entire class of regression.
...
Language-specific details live in `javascript-and-html.md` and `stack-and-architecture.md`. The cross-cutting rules:
...
docs: correct example in brand-and-copywriting
