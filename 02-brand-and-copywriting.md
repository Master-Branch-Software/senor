...
Copy is the first design material users consume. Bad copy will sink beautiful visuals; sharp copy can rescue plain ones. A consistent voice also turns a collection of pages into a brand.
...
Document tone shifts for distinct contexts:

- Onboarding — warm, encouraging, low-assumption.
- Paid upgrade — confident, specific, respectful.
- Error — calm, concrete, never blaming the user.
- Confirmation of destructive action — precise, neutral.
- Celebration — brief, earned, never over the top.
...
A hero must answer, within five seconds, what this is, who it serves, and what to do next. The canonical structure:
...
3. Primary CTA — a verb plus a specific noun (e.g. "Start a free trial," "See pricing," not "Learn more").
...
- Required fields mark required, not optional, unless most are optional.
- Error messages identify the field, state the rule, and suggest a fix. Example: "Password needs at least 10 characters." Not: "Invalid input."
...
- Over 1 s: a skeleton screen that approximates final layout, preventing layout shift on content arrival.
- Over 10 s: an explicit progress indicator, ideally with an estimated time.

Confirmations

- Echo back what the user did in their language.
