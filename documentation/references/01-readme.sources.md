# Sources — 01-readme.md

Citations backing claims in [`01-readme.md`](01-readme.md). Developer artifact — not loaded by skill consumers. Update in the same commit when chapter rules change.
...
The list of AI-fingerprint words ("delve," "leverage," "robust," "seamless," "tapestry," etc.) is cross-referenced with `front-end/references/02-brand-and-copywriting.md` and its sources file. Treat that chapter as the canonical list; keep this chapter's anti-patterns aligned but do not duplicate the full vocabulary here.
...
- The 500 KB image budget is a performance heuristic that aligns with `front-end/references/06-performance.md`; cross-link if that chapter sets a different number.
